...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_hierarchy') {
    try {
        ...
        $user_id = Handler::var('user')['id'];
        $char_name = trim($_POST['character_name'] ?? '');

        if ($char_name !== '') {
            // 1. Create top-level character album with parent_id NULL
            $stmt_new_char = $pdo_gen->prepare("INSERT INTO chv_albums (album_name, album_user_id, album_parent_id, album_date, album_date_gmt, album_creation_ip) VALUES (?, ?, NULL, NOW(), NOW(), '127.0.0.1')");
            $stmt_new_char->execute([$char_name, $user_id]);
...
<div class="content-width">
	...
	<div id="top-user" class="top-user<?php echo !isset(Handler::var('user')["background"]) ? ' no-background' : ''; ?>">
		<div class="top-user-credentials">
			...
			<h1 class="top-user-name"><a href="<?php echo Handler::var('user')['url']; ?>"><?php echo Handler::var('safe_html_user')["name"]; ?></a></h1>
			<?php
                if (Handler::var('user')['is_content_manager']) {
                    ?>
...
<div class="content-width">
    <?php if (Handler::cond('owner') || Handler::cond('content_manager')): ?>
