...
try {
    ...
    foreach ($images as $image) {
        ...
        if (preg_match('#^([0-9]{4})-([0-9]{2})-([0-9]{2})#', $date, $m)) {
            ...
            // Bitmask:
            // 4: only original (00100)
            // 5: original + thumb (00101)
            // 6: original + medium (00110)
            // 7: original + medium + thumb (00111)
            $chainValue = 4;
            if (isset($r2Files[$origPath])) {
                if (isset($r2Files[$thPath]) && isset($r2Files[$mdPath])) {
                    $chainValue = 7;
                } elseif (isset($r2Files[$thPath])) {
                    $chainValue = 5;
                } elseif (isset($r2Files[$mdPath])) {
                    $chainValue = 6;
                }
            }
    ...
    echo "Successfully recalculated chain values for $updatedCount images.\n";
